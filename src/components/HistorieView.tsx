  ...
  return (
    <div className="space-y-7">
      ...
      <section className="space-y-2">
        ...
        <p className="text-[11px] text-slate-100/40">Najeď (nebo klepni) na kartu pro celé pořadí.</p>
...
function StatCard({ icon, label, rows, accent }: { icon: string; label: string; rows: RankRow[]; accent: string }) {
  const [open, setOpen] = useState(false);
  const top = rows[0];
  // jména se shodnou špičkovou hodnotou (remízy) na čele karty
