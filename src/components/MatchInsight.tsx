...
export function MatchInsight({ matchId, onClose }: { matchId: number; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/55 sm:items-center" onClick={onClose}>
      <div
        className="max-h-[86vh] w-full overflow-auto rounded-t-2xl border border-terrain-700 bg-terrain-900 p-4 pb-6 sm:max-w-md sm:rounded-2xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mx-auto mb-3 h-1 w-10 rounded-full bg-terrain-600 sm:hidden" />
                  ...
                  {data.form.map((f) => (
                    <div key={f.matchId} className="flex items-center gap-2 border-b border-terrain-800/60 px-3 py-2.5 last:border-0">
                      <div className="flex min-w-0 flex-1 items-center gap-1.5 text-[12.5px] text-white">
                        <Flag team={f.home} />
                        <span className="min-w-0 truncate">{f.home} <b className="font-display font-bold tabular-nums">{f.hs}–{f.as}</b> {f.away}</span>
                        <Flag team={f.away} />
                      </div>
